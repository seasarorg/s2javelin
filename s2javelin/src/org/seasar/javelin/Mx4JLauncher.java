...
public class Mx4JLauncher {
	public static void execute(MBeanServer server, int httpPort)
		...
		if (server.isRegistered(processorName)) {
			...
			if (beanSet.size() > 0) {
				XSLTProcessor[] processors = (XSLTProcessor[]) beanSet
						.toArray(new XSLTProcessor[beanSet.size()]);
		...
		HttpAdaptor adaptor;
		ObjectName adaptorName = new ObjectName("Adaptor:name=adaptor,port="
				+ httpPort);
	...
	public static void execute(int httpPort)
			throws MalformedObjectNameException,
			InstanceAlreadyExistsException, MBeanRegistrationException,
			NotCompliantMBeanException, IOException {
		MBeanServer server = ManagementFactory.getPlatformMBeanServer();
		execute(server, httpPort);
	}
}
