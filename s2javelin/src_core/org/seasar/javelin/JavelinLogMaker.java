...
public class JavelinLogMaker {
	...
	private static final String[] MESSAGE_TYPES = new String[] { "Call  ",
			"Return", "Read  ", "Write ", "Throw " };
	...
	public static String createJavelinLog(int messageType, long time,
		...
		Invocation caller;
		if (parent == null) {
			caller = new Invocation(null, null, tree.getRootCallerName(),
					"unknown", 0, 0, 0, 0);
		} else {
		...
		if (messageType == ID_THROW) {
			...
			// 例外オブジェクトID
			addToJvnBuffer(StatsUtil.getObjectID(node.getThrowable()),
					jvnBuffer);
		}
		...
		if (messageType == ID_CALL && node.getArgs() != null
			...
			for (int i = 0; i < node.getArgs().length; i++) {
				...
				jvnBuffer.append(StatsUtil.toStr(node.getArgs()[i], config
		...
		if (messageType == ID_THROW) {
			jvnBuffer.append("<<javelin.StackTrace_START>>");
			jvnBuffer.append(NEW_LINE);
			jvnBuffer.append(node.getThrowable().getClass().getName() + ":"
					+ node.getThrowable().getLocalizedMessage());
			jvnBuffer.append(NEW_LINE);
			StackTraceElement[] stacktrace = node.getThrowable()
					.getStackTrace();
