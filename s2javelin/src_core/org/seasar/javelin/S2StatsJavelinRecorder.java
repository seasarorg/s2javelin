...
public class S2StatsJavelinRecorder {
			...
			try {
				...
				if (listener instanceof AlarmListener) {
					...
					JavelinErrorLogger.getInstance().log(
							alarmListenerName + "をAlarmListenerとして登録しました。");
				} else {
					JavelinErrorLogger
							.getInstance()
							.log(
				...
				JavelinErrorLogger.getInstance().log(
						alarmListenerName + "の登録に失敗したため、Alarm通知に利用しません。", ex);
			}
		}
	}
	...
	public static void preProcess(String className, String methodName,
		...
		if (component == null) {
			String name = ObjectNameUtil.createComponentBeanName(className,
					config);
			try {
		...
		if (invocation == null) {
			String name = ObjectNameUtil.createInvocationBeanName(className,
					methodName, config);
			ObjectName objName;
			try {
				objName = new ObjectName(name);
				String processName = VMStatusHelper.getProcessName();
				invocation = new Invocation(processName, objName,
						componentName, className, methodName, config
								.getIntervalMax(), config.getThrowableMax(),
		...
		try {
			...
			if (returnValue != null) {
				...
				if (config.isReturnDetail()) {
					...
					returnString = StatsUtil.buildDetailString(returnValue,
							returnDetailDepth);
	...
	private static void sendExceedThresholdAlarm(String jvnFileName,
			CallTreeNode node) {
	...
	private static void sendExceedThresholdAlarmImpl(String jvnFileName,
			CallTreeNode node) {
