...
public class S2StatsJavelinRecorder {
	...
	/** プラットフォームMBeanServer */
	private static MBeanServer server_ = ManagementFactory
	...
	private static ThreadMXBean threadMBean = ManagementFactory
			.getThreadMXBean();;
	 ...
	 * @return 登録する文字列
	 */
	private static String createInvocationBeanName(String className,
			String methodName, S2JavelinConfig config) {
		return config.getDomain() + ".invocation:type="
				+ InvocationMBean.class.getName() + ",class="
				+ ObjectNameUtil.encode(className) + ",method="
				+ ObjectNameUtil.encode(methodName);
	}

	private static String createComponentBeanName(String className,
			S2JavelinConfig config) {
		return config.getDomain() + ".component:type="
				+ ComponentMBean.class.getName() + ",class="
	...
	public static void preProcess(String className, String methodName,
			Object[] args, S2JavelinConfig config) {
		...
		try {
			...
			Component componentBeanRoot = MBeanManager
					.getComponent(rootClassName);
			...
			if (invocation == null) {
				invocation = new Invocation(objName, componentName, className,
						methodName, config.getIntervalMax(), config
								.getThrowableMax(),
	 ...
	 */
	public static void preProcess(String className, String methodName,
			Object[] args, StackTraceElement[] stacktrace,
			S2JavelinConfig config) {
		synchronized (S2StatsJavelinRecorder.class) {
			// 初期化処理
			if (isInitialized == false) {
	 ...
	 * @param node
	 */
	private static void setNode(CallTreeNode node, CallTree tree,
			StackTraceElement[] stacktrace, Object[] args,
			Invocation invocation, S2JavelinConfig config) {
		try {
			if (node == null) {
				// 初回呼び出し時はコールツリーを初期化する。
				if (tree == null) {
				...
				System.err.println(threadMBean
						.isCurrentThreadCpuTimeSupported());
				System.err.println(threadMBean.isThreadCpuTimeEnabled());
				setStartThreadInfo(node);
			...
			if (config.isLogArgs()) {
				...
				for (int index = 0; index < args.length; index++) {
					if (config.isArgsDetail()) {
						...
						argStrings[index] = StatsUtil.objectDetailPrinter(
								args[index], argsDetailDepth, 0);
					}
	...
	private static void setStartThreadInfo(CallTreeNode node) {
		node.setStartTime(System.currentTimeMillis());
		node.setStartCpuTime(threadMBean.getCurrentThreadCpuTime());
		node.setStartUserTime(threadMBean.getCurrentThreadUserTime());
		ThreadInfo threadInfo = threadMBean.getThreadInfo(Thread
				.currentThread().getId());
		node.setStartBlockedTime(threadInfo.getBlockedTime());
		node.setStartWaitedTime(threadInfo.getWaitedTime());
	}
	...
	public static void postProcess(Object returnValue, S2JavelinConfig config) {
		try {
			...
			setEndThreadInfo(node);
			if (returnValue != null) {
				String returnString;
				if (config.isReturnDetail()) {
					int returnDetailDepth = config.getReturnDetailDepth();
					returnString = StatsUtil.objectDetailPrinter(returnValue,
							returnDetailDepth, 0);
					returnString = StatsUtil.toStr(returnString, config
							.getStringLimitLength());
				} else {
					returnString = StatsUtil.toStr(returnValue, config
							.getStringLimitLength());
				}
			...
			if (parent != null) {
				...
				if (node.getAccumulatedTime() >= config
						.getStatisticsThreshold()) {
					recordTransaction(node);
				}

				// ファイル出力の閾値を超えていた場合に、Javelinログをファイルに出力する。
				if (node.getAccumulatedTime() >= node.getInvocation()
						.getRecordThreshold()) {
	...
	private static void setEndThreadInfo(CallTreeNode node) {
		node.setEndCpuTime(threadMBean.getCurrentThreadCpuTime());
		node.setEndUserTime(threadMBean.getCurrentThreadUserTime());
		ThreadInfo threadInfo = threadMBean.getThreadInfo(Thread
				.currentThread().getId());
		node.setEndBlockedTime(threadInfo.getBlockedTime());
		node.setEndWaitedTime(threadInfo.getWaitedTime());
	}
